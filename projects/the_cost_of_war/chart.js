...
function circlePlotter(input, rangeX, rangeY, radius, pointSize) {
  ...
  plot.sort = function (type) {
    points = points.sort(function (a, b) { return a[type] - b[type] })
  }
...
d3.json("data.js", function (json) {
  ...
  var title = svg.append("text")
              .attr("class", "title")
              .attr("transform", "translate(" + size.x + "," + (size.y-140)+ ")")
              .text("The Cost of War")
  var subtitle = svg.append("text")
              .attr("class", "subtitle")
              .attr("transform", "translate(" + size.x + "," + (size.y-120)+ ")")
  ...
  d3.select(".namesearch")
    .on("input", function (e) {
      if (animation.running || !animation.completed) animation.stop()
  ...
  var circleMouseEvents = {
    ...
    "mouseover": function (d, i) {
      ...
      tooltip.style("visibility", "visible")
        .html("<h3>"+d.name + "</h3>"+
              "<span>Died:</span>" + d.date +  "<br/>" +
              "<span>In:</span>" + d.c + ((!d.c.match(/(Iraq|Afghanistan)/))?(d.loc == "a" ? ", Afghanistan" : ", Iraq"):"") + "<br/>" +
              "<span>At age:</span>" + d.age+ "<br/> "+
              "<span>Branch:</span>" + d.b)
    },
  ...
  function draw(type) {
    ...
    if (type == "date" || type=="initial") {
      data = data.sort(function(a, b) {return new Date(a.date) - new Date(b.date) })
    }
    else {
      data = data.sort(function(a, b) { return a[type] > b[type] ? -1 : a[type] < b[type] ? 1 : 0 })
    }
    ...
    if (type=="initial") circle.classed("initial", true)
  ...
  var animation = (function(){
    ...
    function tick() {
      ...
      if (tickCount === 0)      counterTitle.text("casualty")
    ...
    return {
      ...
      start: function() {
        ...
        d3.selectAll(circles).classed("initial", true)
        tick()
      ...
      stop: function() {
        ...
        d3.select(".bigcircle").style("display", "none")
        svg.selectAll(".title, .subtitle, .animationText").style("display", "none")
