...
d3.json("data.js", function (json) {
  ...
  function draw(filter) {
    ...
    var dataColumn = filter.type

    scales.x = d3.scale.linear()//pow().exponent(0.4)
    ...
    var xAxis = d3.svg.axis().scale(scales.x).orient("bottom").tickFormat(d3.format(filter.tickFormat)),
          yAxis = d3.svg.axis().scale(scales.y).orient("left")

    axisXElement.call(xAxis)
    axisYElement.call(yAxis)
    chart.selectAll(".labelX").text(filter.description)
